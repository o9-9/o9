...
"@
    ...
    Invoke-o99BusyInfo -action "wip" -message "Busy..." -interactive $false

    Write-Host "Target ISO location: $($SaveDialog.FileName)"

    $index = $sync.o99WindowsFlavors.SelectedValue.Split(":")[0].Trim()
    Write-Host "Index chosen: '$index' from $($sync.o99WindowsFlavors.SelectedValue)"

    $copyToUSB = $sync.WPFo99CopyToUsb.IsChecked
    ...
    $scratchDir = $sync.o99ScratchDir.Text
    ...
    if (-not (Test-Path -Path "$mountDir\sources\install.wim" -PathType Leaf) -and (Test-Path -Path "$mountDir\sources\install.esd" -PathType Leaf)) {
        ...
        if ($?) {
            ...
            $msg = "The export process has failed and o99 processing cannot continue"
            Write-Host $msg
            Set-o9Taskbaritem -state "Error" -value 1 -overlay "warning"
            Invoke-o99BusyInfo -action "warning" -message $msg
    ...
    # Detect image version to avoid performing o99 processing on Windows 8 and earlier
    if ((o99-TestCompatibleImage $imgVersion $([System.Version]::new(10,0,10240,0))) -eq $false) {
        $msg = "This image is not compatible with o99 processing. Make sure it isn't a Windows 8 or earlier image."
        $dlg_msg = $msg + "`n`nIf you want more information, the version of the image selected is $($imgVersion)`n`nIf an image has been incorrectly marked as incompatible, report an issue to the developers."
        Write-Host $msg
        [System.Windows.MessageBox]::Show($dlg_msg, "o9", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Exclamation)
        Set-o9Taskbaritem -state "Error" -value 1 -overlay "warning"
        Invoke-o99BusyInfo -action "warning" -message $msg
        return
    }
    ...
    if ((o99-TestCompatibleImage $imgVersion $([System.Version]::new(10,0,21996,1))) -eq $false) {
        $msg = "Windows 10 has been detected in the image you want to process. While you can continue, Windows 10 is not a recommended target for o99, and you may not get the full experience."
        $dlg_msg = $msg
        Write-Host $msg
        [System.Windows.MessageBox]::Show($dlg_msg, "o9", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Exclamation)
    }
    ...
    try {
        ...
        if ($importDrivers) {
            ...
            if (($injectDrivers -and (Test-Path "$($sync.o99DriverLocation.Text)"))) {
                Write-Host "Using specified driver source..."
                dism /english /online /export-driver /destination="$($sync.o99DriverLocation.Text)" | Out-Host
                if ($?) {
        ...
        if ($injectDrivers) {
            $driverPath = $sync.o99DriverLocation.Text
        ...
        o99-RemoveFeatures -UseCmdlets $true
        Write-Host "Removing features complete!"
        Write-Host "Removing OS packages"
        o99-RemovePackages -UseCmdlets $true
        Write-Host "Removing Appx Bloat"
        o99-RemoveProvisionedPackages -UseCmdlets $true
        ...
        if ((o99-TestCompatibleImage $imgVersion $([System.Version]::new(10,0,26100,1))) -eq $true) {
            try {
                if (Test-Path "$scratchDir\Windows\SystemApps\MicrosoftWindows.Client.FileExp_cw5n1h2txyewy\appxmanifest.xml" -PathType Leaf) {
                    ...
                    icacls "$scratchDir\Windows\SystemApps\MicrosoftWindows.Client.FileExp_cw5n1h2txyewy\appxmanifest.xml" /grant "$(o99-GetLocalizedUsers -admins $true):(M)" | Out-Host
        ...
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\System32\LogFiles\WMI\RtBackup" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\DiagTrack" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\InboxApps" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\System32\LocationNotificationWindows.exe"
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Program Files (x86)\Windows Media Player" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Program Files\Windows Media Player" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Program Files (x86)\Windows Mail" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Program Files\Windows Mail" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Program Files (x86)\Internet Explorer" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Program Files\Internet Explorer" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\GameBarPresenceWriter"
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\System32\OneDriveSetup.exe"
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\System32\OneDrive.ico"
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\SystemApps" -mask "*narratorquickstart*" -Directory
        o99-RemoveFileOrDirectory -pathToDelete "$($scratchDir)\Windows\SystemApps" -mask "*ParentalControls*" -Directory
        ...
        {
            if ($sync.o99UserPassword.Password -eq "")
            {
                o99-NewUnattend -userName "$($sync.o99UserName.Text)"
            }
            else
            {
                o99-NewUnattend -userName "$($sync.o99UserName.Text)" -userPassword "$($sync.o99UserPassword.Password)"
            }
        }
        ...
        Write-Host "Copy link to o9.ps1 into the ISO"
    ...
    try {
        ...
        if ($copyToUSB) {
            ...
            o99-CopyToUSB("$($SaveDialog.FileName)")
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Invoke-o99BusyInfo -action "done" -message "Finished!"
            [System.Windows.MessageBox]::Show($msg, "o9", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Information)
        } else {
