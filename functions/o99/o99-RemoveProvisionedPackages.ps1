function o99-RemoveProvisionedPackages() {
    <#
        .SYNOPSIS
        Removes AppX packages from a Windows image during o99 processing

        .PARAMETER UseCmdlets
            Determines whether or not to use the DISM cmdlets for processing.
            - If true, DISM cmdlets will be used
            - If false, calls to the DISM executable will be made whilst selecting bits and pieces from the output as a string (that was how o99 worked before
              the DISM conversion to cmdlets)

        .EXAMPLE
        o99-RemoveProvisionedPackages
    #>
    param (
    ...
    try
    {
        if ($UseCmdlets) {
            ...
            if ($?) {
                ...
                Write-Host "AppX packages could not be obtained with DISM. o99 processing will continue, but AppX packages will be skipped."
                return
            }
        }
    ...
    catch
    {
        ...
        o99-RemoveProvisionedPackages -UseCmdlets $false
    }
}
