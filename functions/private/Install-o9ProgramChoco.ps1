function Install-o9ProgramChoco {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    Install-o9ProgramChoco -Programs @("7zip","chrome") -Action "Uninstall"
    #>
    ...
    function Update-TaskbarProgress {
        ...
        $sync.form.Dispatcher.Invoke([action] { Set-o9Taskbaritem -state $progressState -value ($currentIndex / $totalPrograms) })
    }
    ...
    function Install-ChocoPackage {
        ...
        $installOutputFile = "$env:TEMP\Install-o9ProgramChoco.install-command.output.txt"
    ...
    function Uninstall-ChocoPackage {
        ...
        $uninstallOutputFile = "$env:TEMP\Install-o9ProgramChoco.uninstall-command.output.txt"
    ...
    for ($currentIndex = 0; $currentIndex -lt $totalPrograms; $currentIndex++) {
        ...
        Set-o9ProgressBar -label "$Action $($Program)" -percent ($currentIndex / $totalPrograms * 100)
        $sync.form.Dispatcher.Invoke([action]{ Set-o9Taskbaritem -value ($currentIndex / $totalPrograms)})
    ...
    Set-o9ProgressBar -label "$($Action)ation done" -percent 100
    # Cleanup Output Files
    $outputFiles = @("$env:TEMP\Install-o9ProgramChoco.install-command.output.txt", "$env:TEMP\Install-o9ProgramChoco.uninstall-command.output.txt")
