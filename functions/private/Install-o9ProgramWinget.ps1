Function Install-o9ProgramWinget {
    <#
    .SYNOPSIS
    ...
    Write-Host "---     Configuring winget packages     ---" -ForegroundColor Cyan
    Write-Host "===========================================" -ForegroundColor Cyan

    for ($i = 0; $i -lt $count; $i++) {
        $Program = $Programs[$i]
        $result = $false
        Set-o9ProgressBar -label "$Action $($Program)" -percent ($i / $count * 100)
        $sync.form.Dispatcher.Invoke([action]{ Set-o9Taskbaritem -value ($i / $count)})

        $result = switch ($Action) {
            "Install" {Invoke-Install -Program $Program}
            "Uninstall" {Invoke-Uninstall -Program $Program}
            default {throw "[Install-o9ProgramWinget] Invalid action: $Action"}
        }
