function Invoke-o9UninstallPSProfile {
    <#
    .SYNOPSIS
        # Uninstalls the o9 PowerShell profile then restores the original profile.
    #>
    ...
    Invoke-WPFRunspace -ArgumentList $PROFILE -DebugPreference $DebugPreference -ScriptBlock {
        ...
        # Check if o9 Tech's PowerShell profile is currently available in the PowerShell profile folder.
        if (Test-Path $PSProfile -PathType Leaf) {
            # Set the GitHub repo path used for looking up the name of o9 Tech's powershell-profile repo.
            $GitHubRepoPath = "o9-9/powershell-profile"

            # Get the unique identifier used to test for the presence of o9 Tech's PowerShell profile.
            $PSProfileIdentifier = (Invoke-RestMethod "https://api.github.com/repos/$GitHubRepoPath").full_name

            # Check if o9 Tech's PowerShell profile is currently installed in the PowerShell profile folder.
            if ((Get-Content $PSProfile) -match $PSProfileIdentifier) {
                # Attempt to uninstall o9 Tech's PowerShell profile from the PowerShell profile folder.
                try {
                ...
                # Attempt to uninstall the o9 PowerShell profile from the system.
                try {
                    # Try and remove the o9 PowerShell Profile file from the disk with Remove-Item.
                    Remove-Item $PSProfile

                    # Let the user know that the o9 PowerShell profile has been uninstalled from the system.
                ...
                # Silently cleanup the oldprofile.ps1 file that was created when the o9 PowerShell profile was installed.
                Remove-Item "$env:USERPROFILE\oldprofile.ps1" | Out-Null
            } else {
                # Let the user know that the o9 PowerShell profile is not installed and that the uninstallation was skipped.
                Write-Host "===> o9 Tech's PowerShell Profile Not Found. Skipped Uninstallation. <===" -ForegroundColor Magenta
            }
        } else {
