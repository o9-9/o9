function Invoke-WPFGetInstalled {
    ...
    if (($sync.ChocoRadioButton.IsChecked -eq $false) -and ((Test-o9PackageManager -winget) -eq "not-installed") -and $checkbox -eq "winget") {
        return
    }
    ...
    Invoke-WPFRunspace -ParameterList @(("managerPreference", $managerPreference),("checkbox", $checkbox)) -DebugPreference $DebugPreference -ScriptBlock {
        ...
        $sync.form.Dispatcher.Invoke([action] { Set-o9Taskbaritem -state "Indeterminate" })

        if ($checkbox -eq "winget") {
            Write-Host "Getting Installed Programs..."
            switch ($managerPreference) {
                "Choco"{$Checkboxes = Invoke-o9CurrentSystem -CheckBox "choco"; break}
                "Winget"{$Checkboxes = Invoke-o9CurrentSystem -CheckBox $checkbox; break}
            }
        }
        elseif ($checkbox -eq "tweaks") {
            Write-Host "Getting Installed Tweaks..."
            $Checkboxes = Invoke-o9CurrentSystem -CheckBox $checkbox
        }
