function Invoke-WPFUnInstall {
    ...
    Invoke-WPFRunspace -ArgumentList @(("PackagesToUninstall", $PackagesToUninstall),("ManagerPreference", $ManagerPreference)) -DebugPreference $DebugPreference -ScriptBlock {
        ...
        $packagesSorted = Get-o9SelectedPackages -PackageList $PackagesToUninstall -Preference $ManagerPreference
        ...
        try {
            ...
            if($packagesWinget.Count -gt 0) {
                Install-o9ProgramWinget -Action Uninstall -Programs $packagesWinget
            }
            ...
            Write-Host "│         Uninstalls have finished           │" -ForegroundColor Cyan
            Write-Host "└────────────────────────────────────────────┘" -ForegroundColor Cyan
