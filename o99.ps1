...
function Get-LatestRelease {
    try {
        $releases = Invoke-RestMethod -Uri 'https://api.github.com/repos/o9-9/o9/releases'
...
function Redireo9oLatestPreRelease {
    $latestRelease = Get-LatestRelease
    if ($latestRelease) {
        $url = "https://github.com/o9-9/o9/releases/download/$latestRelease/o9.ps1"
    } else {
        ...
        $url = "https://github.com/o9-9/o9/releases/latest/download/o9.ps1"
    }

    $script = Invoke-RestMethod $url
    # Elevate Shell if necessary
    if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        Write-Output "o9 needs to be run as Administrator. Attempting to relaunch."
