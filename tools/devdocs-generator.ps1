...
function Get-AdditionalFunctionsFromToggle {
    ...
    foreach ($line in $lines) {
        if ($line -match "`"$buttonName`" \{Invoke-(o9[a-zA-Z]+)") {
            return $matches[1]
        }
    }
}
...
"@
        ...
        $jsonLink = "[View the JSON file](https://github.com/o9-9/o9/tree/main/$jsonFilePath)"
...
function Add-LinkAttributeToJson {
    ...
    foreach ($item in $jsonData.PSObject.Properties) {
        ...
        $docLink = "https://o9-9.github.io/o9/$relativePath"
...
$functions = @{}
$itemnametocut = "WPF(o9|Toggle|Features?|Tweaks?|Panel|Fix(es)?)?"
